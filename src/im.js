...
IM.prototype.send = function(data){
...
IM.prototype.offline = function () {
  this.send($pres({type : 'unavailable'}).tree());
};
/**
 * 发送出席信息
 */
...
IM.prototype.probe = function (to) {
  this.send($pres({from:this.jid, to:this.jid, type : 'probe'}).tree());
};
...
IM.prototype.aggreAddFriend = function (jid,cb) {
    this.send($pres({to:jid,type:"subscribed"}));
    this.send($pres({to:jid,type:"subscribe"}));
    
    const iq = $iq({type:"set"}).c('query',{xmlns:"jabber:iq:roster"}).c('item',{jid:jid});
    this.connection.sendIQ(iq,function (result) {
        console.log('同意添加成功');
    });
};
...
IM.prototype.shield_user = function (jid) {
    var user = this.getUserByJid.call(this,jid);
    if(user){
        this.send($pres({type:'unavailable',to:jid}));
        this.send($pres({type:'unsubscribed',to:jid}));
    }
};
/**
 * 取消屏蔽联系人，订阅
